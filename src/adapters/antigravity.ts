...
/**
 * Antigravity-convention "Use this skill when" / "Do not use this skill when"
 * sections that wrap the shared WhySpec instructions.
 */
function getAntigravityUsageHints(command: WhySpecCommand): {
  useWhen: string[];
  doNotUseWhen: string[];
} {
  switch (command) {
...
function renderSkillContent(command: WhySpecCommand): string {
  const hints = getAntigravityUsageHints(command);
...
/**
 * Generate Google Antigravity skill files.
 *
 * Antigravity discovers skills from .agent/skills/<skill-name>/SKILL.md.
 * Each skill needs SKILL.md with YAML frontmatter containing name and description.
 */
export function generateAntigravitySkills(
  projectRoot: string = "",
): GeneratedFile[] {
  ...
  return WHYSPEC_COMMANDS.map((command) => {
    ...
    return {
      path: `${prefix}.agent/skills/whyspec-${command}/SKILL.md`,
      content: `${frontmatter}\n\n${content}`,
    };
  });
}
