...
describe("E2E: init lifecycle", () => {
  it("creates the full whyspec/ directory structure", () => {
    ...
    expect(existsSync(join(tmpDir, "whyspec"))).toBe(true);
    expect(existsSync(join(tmpDir, "whyspec", "changes"))).toBe(true);
    expect(existsSync(join(tmpDir, "whyspec", "archive"))).toBe(true);
    expect(existsSync(join(tmpDir, "whyspec", "debug"))).toBe(true);
  });
  ...
  it("writes config.yaml with valid structure", () => {
    ...
    const configPath = join(tmpDir, "whyspec", "config.yaml");
...
describe("E2E: plan lifecycle", () => {
  it("creates change folder with intent.md, design.md, tasks.md", async () => {
    ...
    const changePath = join(tmpDir, "whyspec", "changes", "add-auth");
  ...
  it("creates .started file for commit range detection", async () => {
    ...
    const startedPath = join(tmpDir, "whyspec", "changes", "timing-test", ".started");
...
describe("E2E: capture lifecycle", () => {
  it("creates ctx_<id>.md in correct XML format", async () => {
    ...
    const changePath = join(tmpDir, "whyspec", "changes", "capture-test");
  ...
  it("--json returns template with Decision Bridge data", async () => {
    ...
    const designPath = join(
      tmpDir, "whyspec", "changes", "json-capture", "design.md",
    );
    writeFileSync(
      designPath,
      `# Design: json-capture
...
describe("E2E: execute lifecycle", () => {
  it("reads plan and reports task progress", async () => {
    ...
    const tasksPath = join(
      tmpDir, "whyspec", "changes", "exec-test", "tasks.md",
    );
    writeFileSync(
      tasksPath,
      `# Tasks: exec-test
...
describe("E2E: full init → plan → capture → execute → show → search lifecycle", () => {
  it("completes the core lifecycle without errors", async () => {
    ...
    expect(existsSync(join(tmpDir, "whyspec", "config.yaml"))).toBe(true);
    expect(existsSync(join(tmpDir, ".gitignore"))).toBe(false);

    // 2. Plan
    await captureOutput(() => planCommand("auth-feature", {}));

    const changePath = join(tmpDir, "whyspec", "changes", "auth-feature");
    expect(existsSync(join(changePath, "intent.md"))).toBe(true);
    expect(existsSync(join(changePath, "design.md"))).toBe(true);
    expect(existsSync(join(changePath, "tasks.md"))).toBe(true);
...
`,
  ...
  it("surfaces the captured story through show and search", async () => {
    ...
    const changePath = join(tmpDir, "whyspec", "changes", "auth-search");
    writeFileSync(
